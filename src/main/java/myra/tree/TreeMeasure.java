...
public enum TreeMeasure {
    ACCURACY {
	@Override
	public double evaluate(Dataset dataset, Tree tree) {
	    ...
	    return hits / (double) dataset.size();
	}
    },
    PESSIMISTIC {
	@Override
     ...
     */
    public abstract double evaluate(Dataset dataset, Tree tree);
	...
	for (int i = 0; i < dataset.size(); i++) {
	    ...
	    int predicted = tree.classify(dataset, i);
