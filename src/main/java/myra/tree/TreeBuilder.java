...
import static myra.interval.C45Split.EPSILON;
...
public class TreeBuilder {
	...
	if ((diversity(dataset, instances) == 1)
		|| ((attribute =
			select(graph,
			       ...
			       parent,
			       index)) == null)
		|| ((conditions = InternalNode
			.branch(graph,
				dataset,
				attribute,
				instances)) == null)) {
	    int prediction = dataset.findMajority(instances, RULE_COVERED);

	    LeafNode leaf =
		    new LeafNode(dataset.attributes()[dataset.classIndex()]
			    .value(prediction), level, prediction);
	...
	if (valid >= 2) {
		...
		if (count[i] == 0) {
		    ...
		    internal.children[i] = new LeafNode(
		    ...
		    int prediction =
			    dataset.findMajority(split[i], RULE_COVERED);
		    ...
		    int majority = dataset.findMajority(split[i], RULE_COVERED);

		    if (TreeStats.error(internal.children[i]) >= (count[i]
			    - distribution[i][majority] - EPSILON)) {
			internal.children[i] = new LeafNode(
							    dataset.attributes()[dataset
								    .classIndex()]
									    .value(majority),
