...
public final class Stats {
     ...
     */
    private final static double deviation[] =
    ...
    static {
        ...
        coefficient = deviation[i - 1] + (deviation[i] - deviation[i - 1])
    ...
    public static double errors(double total, double errors) {
        if (errors < 1E-6) {
            ...
            double pr = (errors + 0.5 + coefficient / 2
                    + Math.sqrt(coefficient
                            * ((errors + 0.5) * (1 - (errors + 0.5) / total)
                                    + coefficient / 4)))
                    / (total + coefficient);
