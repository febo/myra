...
public class TreeBuilder {
	...
	if ((diversity(dataset, instances) == 1) || ((attribute = select(graph,
									 heuristic,
									 dataset,
									 used,
									 instances,
		...
		|| ((conditions = InternalNode.branch(graph,
						      ...
						      instances)) == null)) {
	    Label prediction =
		    new Label(dataset.findMajority(instances, RULE_COVERED));
	...
	if (valid >= 2) {
		...
		if (count[i] == 0) {
		    ...
		    Label prediction = new Label(dataset
			    .findMajority(instances, RULE_COVERED));
		    ...
		    Label prediction = new Label(dataset
			    .findMajority(split[i], RULE_COVERED));
		    ...
		    Label majority = new Label(dataset
			    .findMajority(split[i], RULE_COVERED));
