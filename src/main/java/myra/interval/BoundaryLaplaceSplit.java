...
/**
 * This class creates discrete intervals based on the Laplace accuracy
 * of the intervals.n This implementation only consider boundary threshold
 * values (values occuring between different class values). 
 * 
 * @author Fernando Esteban Barril Otero
 */
public class BoundaryLaplaceSplit
    extends LaplaceSplit
{
    /**
     * Returns attribute conditions representing the discrete intervals
     * for the specified attribute that have provide the higher Laplace
     * accuracy.
     * 
     * @param dataset the current dataset.
     * @param instances the covered instances flags.
     * @param attribute the index of the continuous attribute.
     * 
     * @return attribute conditions representing discrete intervals for the
     *         specified attribute.
     */
    @Override
    public Condition[] multiple(Dataset dataset, Instance[] instances,
            int attribute, int target)
    {
        ...
        for (int i = 1; i < candidates.length; i++)
        {
            ...
            if (candidates[i - 1].classValue != candidates[i].classValue)
            {
                if (candidates[i - 1].value == candidates[i].value)
                {
                    ...
                    while ((threshold > 1)
                        && (candidates[threshold - 1].value == candidates[threshold].value))
                    {
                        weight = candidates[threshold - 1].weight;

                        lowerSize -= weight;
                        lowerFrequency[(int) candidates[threshold - 1].classValue] -=
                            weight;

                        upperSize += weight;
                        upperFrequency[(int) candidates[threshold - 1].classValue] +=
                            weight;
                        ...
                        && (upperSize + PRECISION_10 >= minimum))
                    {
                        ...
                        accuracy =
                            computeAccuracy(accuracy, target,
                                dataset.classLength(), threshold, candidates,
                                conditions, lowerFrequency, lowerSize,
                                upperFrequency, upperSize);
                    }
                    ...
                    && (size + PRECISION_10 >= minimum))
                {
                    ...
                    accuracy =
                        computeAccuracy(accuracy, target,
                            dataset.classLength(), i, candidates, conditions,
                            intervalFrequency, intervalSize, frequency, size);
                }
            }
        }
    ...
    /**
     * Computes the Laplace accuracy of the intervals.
     * 
     * @param accuracy
     * @param target
     * @param classLength
     ...
     * @return
     */
    private double computeAccuracy(double accuracy, int target,
            int classLength, int index, Pair[] candidates,
            Condition[] conditions, double[] lowerFrequency,
            double lowerSize, double[] upperFrequency, double upperSize)
    {
        double lower =
            (lowerFrequency[target] > 0) ? (lowerFrequency[target] + 1)
                / (lowerSize + classLength) : 0.0;

        double upper =
            (upperFrequency[target] > 0) ? (upperFrequency[target] + 1)
                / (upperSize + classLength) : 0.0;

        double intervalAccuracy = Math.max(lower, upper);

        if (((intervalAccuracy - accuracy) > 1e-15)
            || (lower > 0.0 && lower == accuracy && lowerSize > conditions[0].length)
            || (upper > 0.0 && upper == accuracy && upperSize > conditions[1].length))
        {
            ...
            conditions[0].value[0] =
                (candidates[index - 1].value + candidates[index].value) / 2.0;
            System.arraycopy(lowerFrequency, 0, conditions[0].frequency, 0,
                classLength);
            ...
            conditions[1].value[0] =
                (candidates[index - 1].value + candidates[index].value) / 2.0;
            System.arraycopy(upperFrequency, 0, conditions[1].frequency, 0,
                classLength);
        }
