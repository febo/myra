...
public enum ConflictResolution {
    ...
    FREQUENT_CLASS() {
	@Override
	public int resolve(Dataset dataset, Rule[] rules, boolean[] active) {
	...
	public int resolve(Dataset dataset, Rule[] rules, boolean[] active) {
		...
		if (active[i]) {
		    ...
		    double laplace = (frequency[rules[i].getConsequent()] + 1)
			    / (double) (total + dataset.classLength());

		    if (laplace > best) {
			predicted = rules[i].getConsequent();
			best = laplace;
		    }
		}
	    }
	...
	public int resolve(Dataset dataset, Rule[] rules, boolean[] active) {
		...
		if (active[i]) {
		    ...
		    double laplace = (covered[rules[i].getConsequent()] + 1)
			    / (double) (total + dataset.classLength());
     ...
     *         instance.
     */
    public abstract int resolve(Dataset dataset,
				Rule[] rules,
				boolean[] active);
}